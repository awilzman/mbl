...
def bulk_loader(directory,study,prefix,code,tr_names,trials,suffix='.sto'):
    ...
    # Age-match tibia stiffness for those without FE data (JS_S10 -- needs update)
    # and (JS_F11, low chance)
...
def fft_int(time, force, stiff=None, threshold=0.9):
    ...
    fft_mag = np.abs(fft_force)*fft_freq
    cum_sum = np.cumsum(fft_mag)
    if cum_sum[-1] == 0:
        return None
    cum_prc = cum_sum / cum_sum[-1]
    # Find the cumulative sum value where the cumulative percentage exceeds the threshold
    output = cum_sum[np.argmax(cum_prc >= threshold)]
    return output
...
for index, listed_trial in The_List.iterrows():
    ...
    condition_list = (
        ...
        (The_List['Trial No'] == t))
    The_List.loc[condition_list, 'Hip Flexion ROM (deg)'] = hip_flex_rom
    The_List.loc[condition_list, 'Knee Flexion ROM (deg)'] = knee_flex_rom
    The_List.loc[condition_list, 'Ankle Flexion ROM (deg)'] = ankle_flex_rom
    The_List.loc[condition_list, 'Hip Flexion at Contact (deg)'] = hip_flex_init
    The_List.loc[condition_list, 'Knee Flexion at Contact (deg)'] = knee_flex_init
    The_List.loc[condition_list, 'Ankle Flexion at Contact (deg)'] = ankle_flex_init
    The_List.loc[condition_list, 'Max Ankle Reaction Force (BW)'] = max_rxf
    The_List.loc[condition_list, 'Max Reaction Force Rate (BW_s)'] = max_rxf_lr
    The_List.loc[condition_list, 'Max Ankle Contact Force (BW)'] = max_jcf
    The_List.loc[condition_list, 'Max Contact Force Rate (BW_s)'] = max_jcf_lr
    The_List.loc[condition_list, 'FFT RXF FE'] = fft_rxf_fe
    The_List.loc[condition_list, 'FFT JCF FE'] = fft_jcf_fe
    The_List.loc[condition_list, 'FFT RXF'] = fft_rxf
    The_List.loc[condition_list, 'FFT JCF'] = fft_jcf
    The_List.loc[condition_list, 'RXF FE'] = max_rxf_strain
    The_List.loc[condition_list, 'JCF FE'] = max_jcf_strain
    The_List.loc[condition_list, 'Strain MagRate RXF'] = max_rxf_strmagrate
    The_List.loc[condition_list, 'Strain MagRate JCF'] = max_jcf_strmagrate
    The_List.loc[condition_list, 'Daily Impact Score'] = d_imp
    The_List.loc[condition_list, 'IMU FFT'] = imu_fft
...
with open(f'{directory}Data/Jump_Study_DataSummary{date}.txt', "w") as file:
    file.write(table)
if save_data:
    The_List.to_csv(directory+f'Data/Jump_Study_Data{date}.csv',index=False)