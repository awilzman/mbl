...
% eg
% default_directory = 'C:\Users\arwilzman\OneDrive - Worcester Polytechnic Institute (wpi.edu)\Documents\Desktop\Compare_Registered_DICOMs\';
% res = 82;
...
function [bv, bmc, bmd] = compare_dicoms(default_directory,res, ...
    ...
    origins = zeros(size(mask_LCV,3),2);
    for i = first_full_slice:size(mask_LCV,3)
        count = 0;
        for j = 1:size(mask_LCV,1)
            for k = 1:size(mask_LCV,2)
                if mask_LCV(j,k,i) > 0
                    origins(i,1) = origins(i,1) + k;
                    origins(i,2) = origins(i,2) - j; % negated y
                    count = count + 1;
                end
            end
        end
        if count > 0
            origins(i,1) = origins(i,1) / count;
            origins(i,2) = origins(i,2) / count;
        else 
            fig=uifigure;
            uialert(fig,strcat('LCV EMPTY SLICE: ',int2str(i)),'ALERT');
        end
    end
    origins(1:first_full_slice,1) = origins(first_full_slice+1,1);
    origins(1:first_full_slice,2) = origins(first_full_slice+1,2);
    ...
    % Calculate metrics
    % 4 sections, anterior/posterior and medial/lateral (AM,AL,PM,PL) columns
    % 3 masks, time 1, 2, and difference (in order) rows
    bv = zeros(3,4);
    bmc = zeros(3,4);
    bmd = zeros(3,4);
    slices = size(mask_1, 3) - first_full_slice + 1;
    mask_1_am = zeros(size(mask_1, 1),size(mask_1, 2),slices);
    mask_1_al = zeros(size(mask_1, 1),size(mask_1, 2),slices);
    mask_1_pm = zeros(size(mask_1, 1),size(mask_1, 2),slices);
    mask_1_pl = zeros(size(mask_1, 1),size(mask_1, 2),slices);
    mask_2_am = zeros(size(mask_2, 1),size(mask_2, 2),slices);
    mask_2_al = zeros(size(mask_2, 1),size(mask_2, 2),slices);
    mask_2_pm = zeros(size(mask_2, 1),size(mask_2, 2),slices);
    mask_2_pl = zeros(size(mask_2, 1),size(mask_2, 2),slices);
    if medial_left
        for z = 1:slices
            zs = z+(size(mask_1,3)-slices);
            y = -round(origins(zs,2));
            x = round(origins(zs,1));
            mask_1_am(1:y,1:x,z) = mask_1(1:y,1:x,zs);
            mask_1_al(1:y,x+1:end,z) = mask_1(1:y,x+1:end,zs);
            mask_1_pm(1+y:end,1:x,z) = mask_1(1+y:end,1:x,zs);
            mask_1_pl(1+y:end,x+1:end,z) = mask_1(1+y:end,x+1:end,zs);
            mask_2_am(1:y,1:x,z) = mask_2(1:y,1:x,zs);
            mask_2_al(1:y,x+1:end,z) = mask_2(1:y,x+1:end,zs);
            mask_2_pm(1+y:end,1:x,z) = mask_2(1+y:end,1:x,zs);
            mask_2_pl(1+y:end,x+1:end,z) = mask_2(1+y:end,x+1:end,zs);
            mask_1_am(y,1:x,z) = 1;
            mask_1_am(1:y,x,z) = 1;
            mask_1_al(y,1:x,z) = 1;
            mask_1_al(1:y,x,z) = 1;
        ...
        for z = 1:slices
            ...
            y = -round(origins(zs,2));
            x = round(origins(zs,1));
            mask_1_al(1:y,1:x,z) = mask_1(1:y,1:x,zs);
            mask_1_am(1:y,x+1:end,z) = mask_1(1:y,x+1:end,zs);
            mask_1_pl(1+y:end,1:x,z) = mask_1(1+y:end,1:x,zs);
            mask_1_pm(1+y:end,x+1:end,z) = mask_1(1+y:end,x+1:end,zs);
            mask_2_al(1:y,1:x,z) = mask_2(1:y,1:x,zs);
            mask_2_am(1:y,x+1:end,z) = mask_2(1:y,x+1:end,zs);
            mask_2_pl(1+y:end,1:x,z) = mask_2(1+y:end,1:x,zs);
            mask_2_pm(1+y:end,x+1:end,z) = mask_2(1+y:end,x+1:end,zs);
            mask_1_am(y,1:x,z) = 1;
            mask_1_am(1:y,x,z) = 1;
            mask_1_al(y,1:x,z) = 1;
            mask_1_al(1:y,x,z) = 1;
    ...
    [bv(1,1), bmc(1,1), bmd(1,1)] = bv_bmc(mask_1_am,res,calibrate_slope,calibrate_int);
    [bv(1,2), bmc(1,2), bmd(1,2)] = bv_bmc(mask_1_al,res,calibrate_slope,calibrate_int);
    [bv(1,3), bmc(1,3), bmd(1,3)] = bv_bmc(mask_1_pm,res,calibrate_slope,calibrate_int);
    [bv(1,4), bmc(1,4), bmd(1,4)] = bv_bmc(mask_1_pl,res,calibrate_slope,calibrate_int);
    [bv(2,1), bmc(2,1), bmd(2,1)] = bv_bmc(mask_2_am,res,calibrate_slope,calibrate_int);
    [bv(2,2), bmc(2,2), bmd(2,2)] = bv_bmc(mask_2_al,res,calibrate_slope,calibrate_int);
    [bv(2,3), bmc(2,3), bmd(2,3)] = bv_bmc(mask_2_pm,res,calibrate_slope,calibrate_int);
    [bv(2,4), bmc(2,4), bmd(2,4)] = bv_bmc(mask_2_pl,res,calibrate_slope,calibrate_int);
...
function [bv, bmc, bmd] = bv_bmc(mask, res, slope, int)
    ...
    bmd = mean(mask(mask > 1)); % Mean of all values > 1
    only_bv = sum(mask(:) > 1);
    only_bv = only_bv * vox_ed^3;
    constant = only_bv / bv;
    bmd = bmd * constant;
    bmd = bmd*slope + int;
    bmc = bmd * bv;
...
function [new_mask] = rotate_mask(mask, rotationAngle, origins, interpolationMethod)
    ...
    assert(size(origins, 2) == 2, 'Origins must be a 2-column numeric array.');
    ...
    for channelIndex = 1:size(mask, 3)
        ...
        x = x - origins(channelIndex, 1);
        y = y - origins(channelIndex, 2);        
