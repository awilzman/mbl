...
#[tib, tib_id, fem, fem_id] = sd.load_qct('QCT/',studies)
[tib, tib_id, fem, fem_id] = sd.load_qct('C:/Users/arwilzman/Desktop/DL_Data/',studies)
...
data_id = data_id.rename({0:1,1:2,2:3},axis='columns')
# scale each row
...
for i in range(65):
    ...
    t_minmax[0,i] = float(scaler.data_min_[0])
    t_minmax[1,i] = float(scaler.data_max_[0])
...
if 'M' in args.model:
    if args.bone == 'femur':
        if 'HR' in args.model:
            if os.path.exists('HR_fem_mask.pkl'):
                with open('HR_fem_mask.pkl', 'rb') as file:
                    HR_mask_data = pickle.load(file)
            else:
                _, HR_mask_data = sd.load_masks('masks/',
                                                '_fm_integral.txt',data_id,maxes,
                                                args.res,args.highres,args.highresred)
        else:
            if os.path.exists('fem_mask.pkl'):
                with open('fem_mask.pkl', 'rb') as file:
                    mask_data = pickle.load(file)
            else:
                mask_data, _ = sd.load_masks('masks/',
                                             '_fm_integral.txt',data_id,maxes,
                                             args.res,args.highres,args.highresred)
    if args.bone == 'tibia':
        if 'HR' in args.model:
            if os.path.exists('HR_tib_mask.pkl'):
                with open('HR_tib_mask.pkl', 'rb') as file:
                    HR_mask_data = pickle.load(file)
            else:
                _, HR_mask_data = sd.load_masks('masks/',
            ...
            if os.path.exists('tib_mask.pkl'):
                with open('tib_mask.pkl', 'rb') as file:
                    mask_data = pickle.load(file)
            else:
                mask_data, _ = sd.load_masks('masks/',
                                             '_tb_integral.txt',data_id,maxes,
...
if 'M2Q' in args.model:
    ...
    if 'HR' in args.model:
        ...
        del HR_mask_data
        network = bn.HR_mask2qct_net(args.prms,maxes,args.res,args.highres,
...
if 'F' in args.model:
    fe_data = pd.read_csv('fe_data.csv', sep=',', header=None)
    fe_data[[3]]+=1 # left right notation needs to match, my bad
    cols = [1,2,3]
    data_id = pd.DataFrame(data_id)
    fe_data = fe_data.merge(data_id, on=cols, how='left')
    fe_data = fe_data.dropna() # find matching indices, drop missing
    matches = fe_data.index
    mask_data = mask_data[matches,:,:,:] # remove masks without FE
...
if 'M2F' in args.model:
    if args.bone == 'tibia':
        if 'i' in args.model:
            network_code = 'itib_fe_net'
            prm_net = 'tib_prm_net'
            if 'ii' in args.model:
                network_code = 'iitib_fe_net'
                prm_net = 'tib_net'
        else:
            network_code = 'tib_fe_net'
    if args.bone == 'femur':
        if 'i' in args.model:
            network_code = 'ifem_fe_net'
            prm_net = 'fem_prm_net'
            if 'ii' in args.model:
                network_code = 'iifem_fe_net'
                prm_net = 'fem_net'
        else:
            network_code = 'fem_fe_net'
    ...
    if 'i' in args.model:
        ...
        network = bn.informed_mask2fe_net(args.prms,maxes,args.res,args.hidden,
                                          args.mpkernel,args.mpstride,args.ckernel,
                                          args.cpad,args.ch1,args.ch2,args.ch3,
                                          args.dropout).cuda()
    else:
...
if 'GAN' in args.model:
    ...
    if '_' in args.load: # if load previous model
        ...
        netD.load_state_dict(torch.load(d_name+args.load+'.pt'))
        netG.load_state_dict(torch.load(d_name+args.load+'.pt'))
...
# =============================================================================
    if unsup:
        network, losses = tt.train_unsupervised(data, network, args.epochs, 
                                             args.lr,args.decay,args.batch_size,
                                             crit, p_int, device)
        [MAE,MSE,RMSE,R2]=tt.model_eval_unsupervised(data,network)
        x_test = data
        y_test = data
# =============================================================================
#     Supervised models
# =============================================================================
        ...
        if 'HR' in args.model:
            HR_mask_data = np.transpose(HR_mask_data, (3, 0, 1, 2))
            x_train,x_test,y_train,y_test = train_test_split(HR_mask_data,y,
                                                             test_size=args.ttsplit,
                                                             random_state=args.randstate)
        else:
            if 'i' in args.model:
                if 'F' in args.model:
                    # Concatenate and split together
